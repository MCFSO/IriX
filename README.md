...
Minecraft 服务器管理工具。Flutter 构建跨平台 UI，Rust 提供高性能底层支持（压缩、下载、HTTP、数据库、编排、NBT、日志），覆盖实例管理、核心下载、插件市场、备份压缩、文件管理、内网穿透、节点与集群编排、远程数据库、NBT 编辑与 AI 助手等完整工作流。
...
- **节点管理** —— 统一管理 MCSManager 面板与 IriX 本地 Go 守护进程节点，支持控制台 WebSocket、实例快照、Java 运行时、回收站、FRP、账户与集群等新 API
- **集群编排** —— K8s 风格的 MC 服务器编排引擎（Rust orchestrator）：期望状态对账、崩溃自愈、按在线人数弹性扩缩容、跨物理机存档迁移
- **配置文件编辑器** —— 内置 YAML 语法高亮、行号、撤销/重做、注释说明
- **文件管理器** —— 本地与远程文件树的复制、粘贴、移动、重命名，含回收站（7 天自动清理）
- **备份 & 压缩** —— Rust 驱动的 ZIP 并行压缩与解压，支持大文件流式下载（分片 + 断点续传）
- **NBT 编辑器** —— 复刻 AnkiNBT 的通用树 / 物品 / 实体 / 村民交易四领域双模式（高级树 + 简易字段）编辑，支持文件与 RCON 连接运行中服务器，Rust 实现 gzip 大端二进制 ↔ SNBT 双向编解码
...
| 知识库 | milvus-sdk-rust（vector_store FFI，远程 Milvus 存取与相似检索）；embedding 由 Rust HTTP 调 AI 模型 API 生成（Dart 侧组装请求） |
...
├── lib/                     # Flutter 应用层
│   ├── main.dart
│   ├── l10n/                # 多语言（简体中文 / English，gen-l10n）
│   ├── models/              # 数据模型（实例、节点、市场、远程）
│   ├── screens/             # 页面（首页、节点、集群、市场、文件、配置、FRP、AI、数据库、NBT 编辑器等）
│   ├── services/            # API 服务与 FFI 封装（Modrinth、Hangar、MSL、节点、FRP、备份、知识库等）
│   ├── state/               # 状态管理（AppState、NodeState）
│   ├── utils/               # 工具（语法高亮、命名、Apple 风格组件等）
│   ├── widgets/             # 通用组件
│   └── data/                # 静态数据（配置注释说明、服务端核心）
├── rust/                    # Rust 原生库（FFI 动态库）
│   ├── backup/              # ZIP 并行压缩
│   ├── downloader/          # HTTP 流式/分片下载（断点续传）
│   ├── http_client/         # 通用 HTTP 请求（ureq + rustls）
│   ├── file_ops/            # 文件扫描、移动、回收站
│   ├── db_client/           # 远程数据库客户端（MySQL/MariaDB/PostgreSQL/Redis）
│   ├── vector_store/        # 向量知识库（milvus-sdk-rust，远程 Milvus）
