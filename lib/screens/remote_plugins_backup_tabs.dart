...
class _RemotePluginsTabState extends State<RemotePluginsTab> {
  ...
  Future<List<_JarItem>> _listJars(
    String dir,
    {required bool recursive, int depth = 0}) async {
    ...
    for (final entry in data.items) {
      ...
      if (entry.isDirectory) {
        if (recursive) {
          result.addAll(await _listJars(path, recursive: true, depth: depth + 1));
        }
  ...
  /// 下载 .jar 到本地（保存对话框）。
  Future<void> _download(_JarItem item) async {
    setState(() => _busy = true);
    try {
  ...
  /// 删除远程 .jar。
  Future<void> _delete(_JarItem item) async {
...
class _RemoteBackupTabState extends State<RemoteBackupTab> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final zips = data.items
          .where((e) => !e.isDirectory && e.name.toLowerCase().endsWith('.zip'))
          .toList()
  ...
  Future<void> _startBackup() async {
    ...
    try {
      ...
      await widget.client.directDownloadToFile(
        ticket,
        savePath,
        (done, total) {
          if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('备份已保存到 $savePath')),
      );
      await _load();
    } catch (e) {
      if (!mounted) return;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.folder_zip, size: 18, color: theme.colorScheme.primary),
                    const SizedBox(width: 8),
                    Expanded(
